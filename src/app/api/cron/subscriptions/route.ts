...
export async function GET(request: Request) {
    ...
    try {
        ...
        for (const sub of allSubs) {
            ...
            // Alert ONLY if:
            // 1. Is a Trial (status contains 'trial')
            // 2. "Alertar" check is TRUE (sub.alert === true)
            // 3. Days Remaining <= 3 (and >=0 to avoid expired appearing)

            const isTrial = sub.status.toLowerCase().includes('trial');
            const hasAlertCheck = sub.alert === true;
            const isEndingSoon = sub.daysRemaining <= warningDays && sub.daysRemaining >= 0;

            if (isTrial && hasAlertCheck && isEndingSoon) {
                alerts.push(sub);
            }
        }
        ...
        return NextResponse.json({
            ...
            monthlyReportSent: (isFirstOfMonth || forceReport)
        });
...
function generateAlertHtml(subs: Subscription[]) {
    ...
    return `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p>Las siguientes suscripciones requieren tu atención inmediata (Trials por vencer):</p>
            ${listItems}
            <a href="https://coda.io" style="display: block; text-align: center; background: #e11d48; color: #fff; padding: 10px; text-decoration: none; border-radius: 5px; margin-top: 20px;">Ir a Coda</a>
        </div>
    `;
}
...
function generateMonthlyReportHtml(personal: Subscription[], business: Subscription[]) {
    const calcTotal = (subs: Subscription[]) => subs.reduce((sum, s) => {
        // New Logic: Sum only if "Activa" (strict, no trials)
        // Using correct mapped property 'status' from Interface
